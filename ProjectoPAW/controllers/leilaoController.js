...
var Leilao = require("../models/leilao");
...
leilaoController.updateLeilao = function(req,res,next){
	Leilao.findByIdAndUpdate(req.body._id,req.body,{new: true}, function(err,leilao){
		if(err){
			next(err);
		}else{
			res.json(leilao);
...
leilaoController.getLeilaoById = function(req,res){
	res.json(req.leilao);
	Leilao.findById(req.body._id,req.body,{new: true}, function(err,leilao){
		if(err){
			next(err);
		}else{
			res.json(leilao);
